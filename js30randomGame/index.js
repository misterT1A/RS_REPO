...
class Game {
  ...
  draw = () => {
    ...
    for (let i = 0; i < this.botCars.length; i++) {
      if (!this.botCars[i].damage) {
        this.ctx.drawImage(
          this.botCars[i].car,
          this.botCars[i].botcarX,
          this.botCars[i].botcarY
        );
        if (!this.botCars[i].damage) {
        ...
        if (
          ((this.carY <= this.botCars[i].botcarY + this.botCars[i].car.height &&
            this.carY + this.car.height >=
              this.botCars[i].botcarY + this.botCars[i].car.height) ||
            (this.carY + this.car.height >= this.botCars[i].botcarY &&
              this.carY <= this.botCars[i].botcarY)) &&
          ((this.botCars[i].botcarX < this.carX &&
            this.botCars[i].botcarX + this.botCars[i].car.width > this.carX) ||
            (this.botCars[i].botcarX < this.carX + this.car.width &&
              this.botCars[i].botcarX + this.botCars[i].car.width >
                this.carX + this.car.width) ||
            (this.botCars[i].botcarX > this.carX &&
              this.botCars[i].botcarX < this.carX + this.car.width &&
              this.botCars[i].botcarX + this.botCars[i].car.width > this.carX &&
              this.botCars[i].botcarX + this.botCars[i].car.width <
        ...
        this.ctx.save();
        this.ctx.translate(
          this.botCars[i].botcarX + this.botCars[i].car.width / 2,
          this.botCars[i].botcarY + this.botCars[i].car.height / 2
        );
        if (this.damgeRotateBot > -80) {
          ...
          this.ctx.drawImage(
            this.botCars[i].car,
            -this.botCars[i].car.width / 2 / 3,
            -this.botCars[i].car.height / 2 - this.damgeRotateBot / 2
          );
    ...
    if (!this.carFullDamage) {
      if (!this.turn) {
        ...
        this.ctx.save();
        this.ctx.translate(
          this.carX + this.car.width / 2,
          this.carY + this.car.height / 2
        );
      ...
      this.ctx.translate(
        this.carX + this.damageCar.width / 2,
        this.carY + this.damageCar.height / 2
      );
      if (this.damgeRotate < 120) {
        this.damgeRotate += 5;
        ...
        this.ctx.drawImage(
          this.damageCar,
          -this.damageCar.width / 2 / 3,
          -this.damageCar.height / 2 - this.damgeRotate / 2
        );
        this.toDamageSmoke();
        this.ctx.restore();
      } else {
    ...
    this.ctx.save();
    this.ctx.translate(
      -this.arrowSpeed.width * 0.78 + 173,
      -this.arrowSpeed.height / 2 + 811
    );
    this.ctx.rotate((this.deg * Math.PI) / 180);
    this.ctx.drawImage(
      this.arrowSpeed,
      -this.arrowSpeed.width * 0.78,
      -this.arrowSpeed.height / 2
    );
    this.ctx.restore();
  ...
  toDamageSmoke = () => {
    this.ctx.drawImage(
      this.damageSmoke,
      -this.damageCar.width / 2,
      -this.damageCar.height / 2
    );
  ...
  toBotDamageSmoke = (i) => {
    this.ctx.drawImage(
      this.botDamageSmoke,
      -this.botCars[i].car.width / 2,
      -this.botCars[i].car.height / 2 + 40
    );
    if (this.isBotDamageSmoke) {
      this.ctx.drawImage(
        this.botDamageSmoke2,
        -this.botCars[i].car.width / 2,
        -this.botCars[i].car.height / 2 + 40
      );
      this.isBotDamageSmoke = false;
  ...
  toPipeSmokeRight = () => {
    if (this.isPipeSmoke) {
      this.ctx.drawImage(
        this.pipeSmoke1,
        -this.car.width / 2 + this.car.width - 30,
        -this.car.height / 2 + this.car.height
      );
      this.isPipeSmoke = false;
    }
    setTimeout(() => {
  ...
  toTireSmokeRight = () => {
    if (this.isTireSmoke) {
      this.ctx.drawImage(
        this.tireSmokeLeft,
        -this.car.width / 2 - 90,
        -this.car.height / 2 + this.car.height - 120
      );
  ...
  drawSpeed = () => {
    if (this.speedTogle) {
      this.ctx.drawImage(
        this.speedEffectDown,
        0,
        this.cvs.height - this.speedEffectUp.height
      );
  ...
  getDate = () => {
    ...
    const date =
      mainDate.getDate() < 10 ? `0${mainDate.getDate()}` : mainDate.getDate();
    const mounth =
      mainDate.getMonth() < 10
        ? `0${mainDate.getMonth()}`
        : mainDate.getMonth();
    const hours =
      mainDate.getHours() < 10
        ? `0${mainDate.getHours()}`
        : mainDate.getHours();
    const minutes =
      mainDate.getMinutes() < 10
        ? `0${mainDate.getMinutes()}`
        : mainDate.getMinutes();
        ...
        if (user.name === this.name) {
          ...
          user.points.forEach((elemScore, indexScore) => {
            this.recordsElements[
              indexScore
            ].textContent = `${elemScore.time}______Score: ${elemScore.score}`;
          });
        }
      });
